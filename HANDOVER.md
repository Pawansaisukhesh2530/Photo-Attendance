...
Panorama mode locks the camera screen to portrait and guides the user through an approximately 120-degree sweep. It automatically captures seven overlapping frames only after the phone reaches the next angle, is level, and remains stable for 300 ms. Zoom and continuous torch settings are locked after a sweep starts so every frame has consistent camera geometry and lighting. The backend contract is unchanged: the frontend uploads the seven source files to `POST /api/v1/attendance/panorama/frames`, then attaches the returned draft to a normal attendance session. The backend stitches those frames into one classroom image before face detection. Motion permission is requested on iOS; Android reads its available rotation and acceleration sensors directly because Expo otherwise maps this call to the unrelated Physical activity permission. Gallery upload remains available for testing when a camera is unavailable.

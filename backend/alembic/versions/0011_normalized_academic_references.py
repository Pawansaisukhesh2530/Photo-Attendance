...
def upgrade() -> None:
    ...
    student_columns = {x["name"] for x in sa.inspect(bind).get_columns("students")}
    ...
    if "mapping_note" not in student_columns:
    ...
    class_columns = {x["name"] for x in sa.inspect(bind).get_columns("classes")}
    if "program_subject_id" not in class_columns:
    ...
    if "section_id" not in class_columns:

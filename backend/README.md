...
The local threshold is `0.60`. OpenCV YuNet detects on a 2048-pixel bounded copy. Face crops smaller than 80 pixels are upscaled 2× before SFace embedding; normal-sized faces keep the original embedding path, and boxes are mapped back to the original photo. Calibrate production thresholds on representative, consented classroom images before deployment. Local PostgreSQL may run with `EDUTRACE_PGVECTOR_ENABLED=false`; production should enable the pgvector extension.
## Capacity benchmark

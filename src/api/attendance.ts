...
export const attendanceApi:AttendanceService={
  ...
  async getAttendanceHistory(query){const raw=await request<{items:{id:string}[]}>('attendance/sessions',{query:{classId:query?.classId,facultyId:query?.facultyId,schoolId:query?.schoolId,departmentId:query?.departmentId,programId:query?.programId,batchId:query?.batchId,sectionId:query?.sectionId,from:query?.from,to:query?.to,status:query?.status,search:query?.search,pendingReviewOnly:query?.pendingReviewOnly,page_size:100}});return Promise.all(raw.items.map(async x=>summary(await getSession(x.id))))},
  async getPagedAttendanceHistory(query){const raw=await request<{items:{id:string}[];page:number;page_size:number;total:number;has_more:boolean}>('attendance/sessions',{query:{classId:query?.classId,facultyId:query?.facultyId,schoolId:query?.schoolId,departmentId:query?.departmentId,programId:query?.programId,batchId:query?.batchId,sectionId:query?.sectionId,from:query?.from,to:query?.to,status:query?.status,search:query?.search,pendingReviewOnly:query?.pendingReviewOnly,page:query?.page,page_size:query?.pageSize}});const items=await Promise.all(raw.items.map(async x=>summary(await getSession(x.id))));return {items,page:raw.page,pageSize:raw.page_size,total:raw.total,hasMore:raw.has_more} as Paginated<AttendanceSessionSummary>},
};

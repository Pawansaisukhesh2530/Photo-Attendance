...
type Raw={institution_name:string;attendance_threshold:number;image_retention_days:number;departments?:string[];version:number;updated_at?:string|null};
const map=(r:Raw):InstitutionSettings=>({institutionName:r.institution_name,institutionCode:'EDU',attendanceThreshold:r.attendance_threshold,academicSession:`${new Date().getFullYear()}-${String(new Date().getFullYear()+1).slice(-2)}`,departments:r.departments??['CSE'],semesterCount:8,allowPostFinalizationEdits:true,updatedAt:r.updated_at??null,updatedBy:null,updatedByName:null});
const body=(p:UpdateSettingsRequest,version:number)=>({institution_name:p.institutionName,attendance_threshold:p.attendanceThreshold,departments:p.departments,version});

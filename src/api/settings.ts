...
type Raw={institution_name:string;attendance_threshold:number;image_retention_days:number;departments?:string[];faculty_roles?:string[];version:number;updated_at?:string|null};
...
const body=(p:UpdateSettingsRequest,version:number)=>({institution_name:p.institutionName,attendance_threshold:p.attendanceThreshold,departments:p.departments,faculty_roles:p.facultyRoles,version});

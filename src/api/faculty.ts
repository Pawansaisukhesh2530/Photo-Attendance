...
export const facultyApi:FacultyService={
 ...
 createFaculty:p=>request<Faculty>('faculty',{method:'POST',body:{email:p.email,employee_id:p.employeeId,name:p.name,department:p.department,designation:p.designation}}),
 async updateFaculty(p){const old=await request<WithVersion>(`faculty/${p.facultyId}`);return request<Faculty>(`faculty/${p.facultyId}`,{method:'PATCH',body:{name:p.name,email:p.email,department:p.department,designation:p.designation,status:p.status,version:old.version}})},

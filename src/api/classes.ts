...
type Raw={id:string;code:string;variant:string;subject:string;department:string;semester:number;section:string;academic_session:string;archived:boolean;version:number;faculty_id?:string|null;faculty_name?:string|null;student_count?:number;attendance_percentage?:number;schedule?:Array<{day_of_week:number;day_label:string;start_time:string;end_time:string;room:string|null}>};
...
export const classesApi:ClassService={
 ...
 createClass:async p=>map(await request<Raw>('classes',{method:'POST',body:{code:p.classCode,variant:p.variant??'Lecture',subject:p.subject,department:p.department,semester:p.semester,section:p.section,academic_session:p.academicSession}})),
 async updateClass(p){const old=await getRaw(p.classId);return map(await request<Raw>(`classes/${p.classId}`,{method:'PATCH',body:{subject:p.subject,variant:p.variant,department:p.department,semester:p.semester,section:p.section,academic_session:p.academicSession,archived:p.status==='ARCHIVED'?true:p.status==='ACTIVE'?false:undefined,version:old.version}}))},

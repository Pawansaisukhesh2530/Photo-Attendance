...
export function AcademicHierarchyFields({value,onChange,through='sections',locked={}}:{value:HierarchySelection;onChange:(value:HierarchySelection)=>void;through?:HierarchyLevel;locked?:Partial<Record<keyof HierarchySelection,boolean>>}){
 ...
 const options=useMemo(()=>!open||!tree.data?[]:tree.data[open].filter(row=>row.active&&(!current?.parentField||row[current.parentField]===value[current.parent!])).map(row=>({id:row.id,label:row.name,description:row.code??row.path?.map(part=>part.name).join(' / '),selected:row.id===value[current!.id]})),[open,tree.data,current,value]);
